...
func GenerateFreeSSLCerts(domain string, email string) *certificate.Resource {
	...
	providerConfig.AuthToken = viper.GetString("digitalocean.authToken")
...
func OpenDb() *sql.DB {
	config := mysql.Config{
		User:                    viper.GetString("aws.dbUser"),
		Passwd:                  viper.GetString("aws.dbPassword"),
		Net:                     "tcp",
		Addr:                    viper.GetString("aws.dbEnpoint"),
		DBName:                  viper.GetString("aws.dbName"),
...
func UpdateCertsInDb(certificates *certificate.Resource, domain string) error {
	db := OpenDb()
	// UPDATE domain_cert_table SET private_key, issuer_cert, cert WHERE domain = '%s'
